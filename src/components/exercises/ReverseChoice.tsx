...
import { ReverseChoiceQuestion } from "@/lib/exercises";

interface Props {
  question: ReverseChoiceQuestion;
  onAnswer: (correct: boolean, userAnswer?: string) => void;
  selected: string | null;
  onSelect: (answer: string) => void;
}

export default function ReverseChoice({ question, onAnswer, selected, onSelect }: Props) {
  ...
  return (
    <div>
      <div className="bg-card border border-card-border rounded-2xl p-8 text-center mb-5">
        <p className="text-[10px] text-muted tracking-wide uppercase mb-3">Выберите японский вариант</p>
        <div className="text-2xl font-bold">{question.prompt}</div>
      </div>
